...
class MyRequestRepo extends BaseRepo {
    ...
    public function createFromSales($data)
    {
        $itemsRequestRepo = new ItemsRequestRepo();
        ...
        for($i=0; $i < $cant ; $i++)
        {
            $newIR = ['items_id'=> $data['items_id'], 'my_request_id'=> $model->id , 'branches_to_id'=> Auth::user()->branches_active_id ,'status'=> 4, 'types_id' => 1 ];
            $itemsRequestRepo->create($newIR);
        }
