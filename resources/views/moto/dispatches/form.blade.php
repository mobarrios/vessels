    ...
    @if(isset($models))
        <div class=" panel-default col-xs-6">
            ...
            <table class="table table-striped">
                <thead><th>Detalle Pedido # {{$models->PurchasesOrders->id}}</th></thead>
                <tbody>
                @foreach($models->PurchasesOrders->PurchasesOrdersItems as $purchasesOrder)
                    <tr>
                        <td><span class="text-muted">cant.</span> {{$purchasesOrder->quantity}}</td>
                        <td>{{$purchasesOrder->Models->Brands->name}} <strong>{{$purchasesOrder->Models->name}}</strong></td>
                        <td><span class="text-muted">color.</span>{{$purchasesOrder->Colors->name}} </td>
                    </tr>
                @endforeach
                </tbody>
