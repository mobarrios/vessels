                    ...
                    ?>
                        @foreach($models->PurchasesOrders as $purchase)
                            <tr>
                                <td>{{$purchase->date}}</td>
                                <td>Deuda</td>
                                <td><a href=" {{route('moto.purchasesOrders.edit',$purchase->id)}}"> Pedido de Mercadería # {{$purchase->id}} </a></td>
                                <td></td>

                                @foreach($purchase->PurchasesOrdersItems as $item)
                                        <? $debe = $debe + ($item->quantity * $item->price) ?>
                                @endforeach
